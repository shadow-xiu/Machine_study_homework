...
class TEProcessDataset(Dataset):
    def __init__(self, file_paths, seq_len=10, use_pca=False, n_components=5):
        self.data = []
...
# 创建数据集，并启用PCA降维
train_dataset = TEProcessDataset(train_files, use_pca=True, n_components=12)  # 使用PCA降维
test_dataset = TEProcessDataset(test_files, use_pca=True, n_components=12)  # 使用PCA降维
...
input_size = train_dataset.data.shape[1] - 1  # 特征数（PCA降维后的特征数）
hidden_size = 64
num_layers = 2
output_size = 1
...
for epoch in range(num_epochs):
    ...
    print(f'Epoch [{epoch+1}/{num_epochs}], 损失: {epoch_loss:.4f}')
